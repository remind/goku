...
public class PageBuilder {
	 ...
	 * @return
	 */
	public static Page build(String sql, int perPageNumber, int currentPageNumber, SqlParam sqlParam) {
		PageBuilder builder = new PageBuilder(sql, perPageNumber, currentPageNumber);
		builder.setSqlParam(sqlParam);
		builder.execute();
		Page page = new Page(builder.getData(), builder.currentPageNumber,builder.totalPageNumber,builder.totalRecordNumber);
		return page;
	}
	...
	public void execute() {
		...
		Map<String, Object> map = Db.findSingle(totlaSql, sqlParam);
		...
		data = Db.find(querySql, sqlParam);
	}
